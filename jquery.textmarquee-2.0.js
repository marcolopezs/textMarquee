...
(function($) {
	...
	$.fn.textMarquee.init = function($textMarquee) {
		...
		if(settings.matchWidthOfWrapperOnResize == true) {
			...
			$(window).resize(function() {
				...
				_debug("window resized:" + parentWidth);
			});
		};
	...
	$.fn.textMarquee.wordsObject = {
		fetchData: function($textMarquee, settings) {
								...
								$.ajax({
									...
									success: 	function(data) {
													_debug('success! ajax request complete.');
		...
		processData: function(data, $textMarquee, settings) {
			...
			// wrapper object 
			if(data) {
				// a simple processor to get us to the root of the data object
				// first level, if we get to a string, then we are deep enough. 
				if(typeof data[1].word === 'string') {
					// pass data in
					$.fn.textMarquee.wordsObject.words = data;
					console.log("processData data[0].word");
					return true;
				} else if(typeof data[1][0] === 'string') {
					// one level deeper
					$.fn.textMarquee.wordsObject.words = data[0];
					console.log("processData data[0][0].word");
					return true;
				} else if(typeof data[1][0][0] === 'string') {
					// one level deeper!
					$.fn.textMarquee.wordsObject.words = data[0][0];
					console.log("processData data[0][0][0].word");
					return true;
				} else {
					_debug('The processData method expects a simplified object. Please provide an array of words.');
					console.log("processData: fail");
					return false;
				}
			} else {
				_debug("processData: data not available");
				_debug(data);
			}
			console.log('data processed:');
			console.log($.fn.textMarquee.wordsObject.words);
		},
		words: []
	...
	$.fn.textMarquee.createRows = function($textMarquee) {
		
		_debug('create rows called');
		
		var settings = this.settings;
		
		for(var i=0; i < settings.rows; i++) {
			
			_debug('generate row: '+i);
			...
			// row wrapper
			var $row = $('<'+settings.rowWrapper+' class="row ' + evenOdd + '">');
	...
	$.fn.textMarquee.writeWordsToRows = function($textMarquee) {
		...
		_debug('write words to rows called');
		...
		$.each($targets, function(index, target) {
			...
			$.each($words, function(index, word) {
				_debug('write word to copied row: '+word);
				var $word = $(word);
			...
			// set $target width before cloning    d
			$target.css({
	...
	$.fn.textMarquee.animateRows = function($textMarquee) {
		...
		// randomize speed variant
		$.each(rows, function(index, row) {
			
			_debug('randomize speed: '+index);
